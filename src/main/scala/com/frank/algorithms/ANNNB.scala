...
case class ANNNB(architecture:List[Int],actFunc:Double=>Double) {
  ...
  def initialize :List[DenseMatrix[Double]] ={
    ...
    for (l <- 0 until n_levels-1) {
       thetas += DenseMatrix.rand(architecture(l+1),architecture(l)).map(_*2-1)
    }
    thetas.toList
  }
  ...
  def forwardProp(x:DenseVector[Double],thetas: List[DenseMatrix[Double]]) : (DenseVector[Double],List[DenseVector[Double]],List[DenseVector[Double]]) = { 
    ...
    def getOutput(spikes: List[DenseVector[Double]],thetas: List[DenseMatrix[Double]],acts : List[DenseVector[Double]]) : 
                 (DenseVector[Double],List[DenseVector[Double]],List[DenseVector[Double]]) = {
      val activation = spikes.last.map(actFunc)
  ...
  def backProp(y_hat:DenseVector[Double],y:DenseVector[Double],thetas:List[DenseMatrix[Double]],acts:List[DenseVector[Double]]): List[DenseVector[Double]] ={
    ...
    def getDelta(deltas:List[DenseVector[Double]],thetas:List[DenseMatrix[Double]], as:List[DenseVector[Double]]) :List[DenseVector[Double]] ={
      ...
      if(thetas.isEmpty) deltas.tail
        ...
        val del = (thetas.last.t * deltas.head) :* as.last :* (as.last.map(1.0-_)) 
        //val del = (thetas.last.t * deltas.head) :* as.last.map(sigmoid(_)) 
        getDelta(del +: deltas, thetas.init, as.init)
      }
    }
    getDelta(List(y_hat-y),thetas,acts.init)
  }
  ...
  def get_gradient(thetas: List[DenseMatrix[Double]],x:Array[DenseVector[Double]], y:Array[DenseVector[Double]],lambda_reg:Double) :List[DenseMatrix[Double]] ={
    ...
    if(x.size != y.size) {println("size of X does NOT match that of y");break}
        ...
        val big_ds=  x.zip(y).par.map{case (a,b)=> 
  ...
  def optimize_gradient(thetas: List[DenseMatrix[Double]],x:Array[DenseVector[Double]], y:Array[DenseVector[Double]],lambda:Double,lambda_reg:Double,n_iter:Int) : List[DenseMatrix[Double]] ={
    def update_gradient(thetas: List[DenseMatrix[Double]],x:Array[DenseVector[Double]], y:Array[DenseVector[Double]],lambda:Double,lambda_reg:Double,iter:Int) : List[DenseMatrix[Double]] ={
      ...
      if(iter <= n_iter){
        val big_ds = get_gradient(thetas,x,y,lambda_reg)
        update_gradient( update_thetas(big_ds,thetas,lambda)  ,x,y,lambda,lambda_reg,iter+1)
      }
      else thetas
  ...
  def get_cost_regularized(X:Array[DenseVector[Double]], y:Array[DenseVector[Double]],thetas: List[DenseMatrix[Double]],lambda_reg:Double) :Double = {
    if(X.size != y.size) {println("size of X does NOT match that of y");break}
      ...
      cost += X.zip(y).par.map{case (x,y)=> 
        val (pred,_,_)=this.forwardProp(x,thetas)
        (pred-y).sumOfSquares}.sum
